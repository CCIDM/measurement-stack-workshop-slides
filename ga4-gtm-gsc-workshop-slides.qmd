...
format:
  revealjs:
    ...
    slide-number: true
    preview-links: true
    logo: "images/cpp-logo.png"  # <-- replace with your actual logo path
    center: false
    progress: true
    controls: true
...
```{=html}
<style>
/* Utility color classes (match custom.scss variables) */
.text-cpp-green { color: #005030; }
.text-cpp-gold  { color: #FFB81C; }
.text-avocado   { color: #A4D65E; }
.text-citrus    { color: #FF7500; }
.text-teal      { color: #00838A; }
.text-rose      { color: #b52b4c; }
.text-black     { color: #121212; }

/* Small highlight badges (works alongside .highlight in custom.scss) */
.badge { padding: 0.08em 0.35em; border-radius: 0.25em; color: #FFFFFF; }
.badge-green { background: #005030; }
.badge-citrus { background: #FF7500; }

/* Visual placeholders (swap with screenshots as you build the deck) */
.placeholder-box {
  border: 3px dashed #CFBAB0;
  border-radius: 12px;
  padding: 0.9em;
  min-height: 220px;
  display: flex;
  ...
  font-size: 0.95em;
  color: #121212;
  background: rgba(226,232,224,0.4);
}
.placeholder-tall { min-height: 320px; }

/* Optional: compact ULs on dense slides (custom.scss sets ULs large) */
.compact ul { font-size: 1.6rem !important; }
.compact ul ul { font-size: 1.3rem !important; }
.compact ul ul ul { font-size: 1.1rem !important; }
</style>
```
...
::: {.notes}
- Emphasize this is a live build + validation session, not a lecture-only session.
- Encourage “follow along” and to pause you when they hit a mismatch.
:::
...
```text
include-in-header.html  ← paste the <script> snippet
include-before-body.html ← paste the <noscript> snippet
```
